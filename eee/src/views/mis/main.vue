<template>
  <div class="site-wrapper" :class = "{ 'site-sidebar--fold': sidebar.sidebarFold }" v-loading = "fullscreen.lock"
       element-loading-text="拼命加载中">
    <nav class="site-navbar" :class = "{ 'site-navbar--fold': sidebar.sidebarFold }">
      <div class="site-navbar__header">
        <h1 class="site-navbar__brand">
          <a class="site-navbar__brand-lg">清逸体检系统</a>
    ...
    <aside class="site-sidebar site-sidebar--dark">
      <div class="site-sidebar_inner">
        <el-menu :default-active="siteContent.menuActiveName || 'Home'" :collapse="sidebar.sidebarFold"
                 :collapseTransition="false" class="site-sidebar menu" background-color="#263238" active-text-color="#fff"
                 text-color="#8a979e">
          ...
          <el-sub-menu index="组织管理"
            ...
            <template #title>
              <el-icon>
                <SvgIcon name="users fill" class="icon-svg" />
              </el-icon>
            ...
            <el-menu-item index="MisRole" v-if="proxy.isAuth(['ROOT', 'ROLE:SELECT'])" @click = "$router.push({ name: 'MisRole' })">
              <el-icon>
                <SvgIcon name="role_fill" class="icon-svg" />
              </el-icon>
              <span slot="title">角色管理</span>
            </el-menu-item>
            <el-menu-item index="MisUser" v-if="proxy.isAuth(['ROOT', 'USER:SELECT'])" @click = "$router.push({ name: 'MisUser' })">
              <el-icon>
          ...
          <el-sub-menu index="业务管理"
            ...
            <template #title>
              <el-icon>
                <SvgIcon name="trust fill" class="icon-svg" />
              </el-icon>
              <span slot="title">业务管理</span>
            </template>
            <el-menu-item index="MisGoods"
                          v-if="proxy.isAuth(['ROOT', 'GOODS:SELECT'])" @click = "$router.push({ name: 'MisGoods' })">
              <el-icon>
                <SvgIcon name="goods_fill" class="icon-svg" />
              </el-icon>
              <span slot="title">体检套餐</span>
            </el-menu-item>
            <el-menu-item index="MisRule" v-if="proxy.isAuth(['ROOT', 'RULE:SELECT'])" @click = "$router.push({ name: 'MisRule' })">
              <el-icon>
          ...
          <el-sub-menu index="体检管理"
            ...
            <el-menu-item index="MisCustomerCheckin" v-if="proxy.isAuth(['ROOT', 'CUSTOMER_CHECKIN:SELECT'])"
            ...
            <el-menu-item index="MisAppointmentRestriction" v-if="proxy.isAuth(['ROOT', 'APPOINTMENT_RESTRICTION:SELECT'])"
          ...
          <el-sub-menu index="系统设置"
            ...
            <el-menu-item index="MisFlowRegulation" v-if="proxy.isAuth(['ROOT', 'FLOW_REGULATION:SELECT'])"
              ...
              <el-icon>
                <SvgIcon name="people fill" class="icon-svg" />
              </el-icon>
